% 对于单车道交通流的模拟：
% 时间、空间、车辆速度都被整数离散化。单路被划分为等距离的格子，即元胞。每个元胞或者是空的，或者被一辆车占据。车辆速度可以在（0~Vmax）之间取值
% 规则如下：
% 1.加速：V_n->min(V_n +1,V_max)
% 2.减速：V_n->min(V_n,d_n)
% 3.随机慢化：以随机概率p进行慢化 V_n->min(V_n -1,0)
% 4.位置更新：V_n->X_n+V_n
...
%     rng('state',sum(100*clock)*rand(1));
    p_rand=rand; %产生随机概率
    if p_rand<=p
        v=max(v-1,0);
    end
